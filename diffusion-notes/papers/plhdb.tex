...
\begin{eqnarray}
     \hat{u}^{[0]} = \sum_{j=1}^{N}\chi_j(\xi U_j+(1-\xi)U_{j-1})
...
$[\D x{\hat{u}^{[0]}}]_j=\delta^2U_j/H$. Hence, we introduce a homotopic smoothing parameter $\gamma$
($0\le\gamma\le 1$),
...
Consequently, the state-space is now extended to $(u,\alpha,\gamma)$, for which $(\hat{u}^{[0]},0,0)$ is
