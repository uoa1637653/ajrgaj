...
\let\harvardurl\url
\usepackage{graphicx,amsmath,amsfonts,moreverb,microtype}
\usepackage{mathrsfs}
\usepackage{epstopdf}
\usepackage{enumitem}
\allowdisplaybreaks
...
\crefname{enumiii}{}{}
\crefname{enumiv}{}{}
% then mydefs including theorems et al.
\usepackage{defns}
...
\Cal E \Cal I \Cal L \Cal M \Cal N \Cal R \Cal V \Cal X \Cal Y \Cal Z

\newcommand{\secref}[1]{\cref{sec:#1}}

\title{Smooth subgrid fields underpin rigorous closure in spatial discretisation of reaction-advection-diffusion PDEs}
...
As a concrete example, we demonstrate the holistic discretisation procedure on the well-known Burgers' \pde{}, and compare the theoretical and numerical stability of the resulting discretisation to other approximations.
...
Precise requirements on the class of functions~\(F\) and~\(G\) are identified in \cref{thm:gsm} (\S\ref{sec:lin}).
Although most of this article addresses \pde{}s of the form~\eqref{eq:genpde}, \cref{sec:smwpde} discusses generalising the theoretical support to wave-like \pde{}s obtained by replacing~\(u_t\) by~\(u_{tt}\) in~\eqref{eq:genpde}.
...
\label{eq:csd}
\end{equation}
We use centre manifold theory \cite[e.g.]{Carr81, Haragus2011} to do the following (\cref{sec:lin}): 
...
This new theory is applied in \cref{sec:nonlin,sec:numeric} to construct and evaluate the new approach for the classic example of the nonlinear advection--diffusion Burgers' \pde
\begin{equation}
	\D tu = \nu\DD xu - \alpha u\D xu\,,
\label{eq:burgers}
\end{equation}
for both diffusion~\(\nu\) and nonlinearity parameter~\(\alpha\) being real and constant.
This \pde\ is commonly used as a first test of such discretisation schemes \cite[e.g.]{Dhawan2012}.
...
Initial conditions for the discrete closure~\eqref{eq:csd} are nontrivial.
The paradox is that despite the definition~\eqref{eq:gridval} that~\(U_j(t)=u(X_j,t)\), in general the initial grid value \(U_j(0)\neq u(X_j,0)\) for accurate forecasts over long-times.
In physics, \cite{vanKampen85} called this phenomena ``initial slip'', and it is effectively a `boundary layer' in time that accounts for rapid  transients.
\cite{Roberts89b} developed an efficient general method that would algebraically derive the correct initial~\(U_j(0)\) for accurate long-time forecasts---the method has been successful in various other applications \cite[e.g.]{Roberts2014a}.

The spatial domain~\(\XX\) of the \pde~\eqref{eq:genpde} is of length~\(L\), \(0\leq x\leq L\)\,, and we mostly restrict attention to solutions~\(u(x,t)\) which are \(L\)-periodic in space, but occasionally comment on the cases of homogeneous Dirichlet boundary conditions, \(u(0,t)=u(L,t)=0\)\,, and Neumann boundary conditions, \(u_x(0,t)=u_x(L,t)=0\)\,.
...
The multiscale derivation of the so-called stabilized schemes \cite[e.g.]{Hughes95} appears analogous to the first step of the construction described by \cref{sec:nonlin}.
...
New theory established in \cref{sec:lin} asserts that in principle an \emph{exact} closure~\eqref{eq:csd} exists (a slow manifold); that is, there is some system of \ode{}s~\eqref{eq:csd} that gives exact solutions of the \pde~\eqref{eq:genpde}.
...
For illustrative purposes, \cref{sec:nonlin} compares results with Burgers' \pde~\eqref{eq:burgers} on a periodic domain discretised to the so-called mixture model
\begin{equation}
	\dot{U}_j = 
...
\label{eq:mixture}
\end{equation}
for \(N\)-periodic index~\(j\).
In contrast, \secref{nonlin} shows our holistic approach has no such representational ambiguity, and constructs at first-order the specific model
\begin{equation}
	\dot{U}_j = S\left[
...
\secref{numeric} further compares the numerical behaviour of our holistic and established mixture models.
...
That is, as illustrated by the two approximate examples of \cref{fig:uhat}, we construct the field (a slow manifold)
\begin{equation}
	u(x,t)  = {u}(x,t,{\Uv(t)}),
	\label{eq:spatial}
\end{equation}
...
The complete holistic framework comprises equations~\eqref{eq:csd} and~\eqref{eq:spatial},  in conjunction with suitable boundary 
and  inter-element coupling conditions to be specified  
in more detail in \secref{lin}. 


In particular, the Rayleigh--Ritz theorem motivates coupling conditions that give a piecewise linear function as the leading approximation (e.g., the blue~\(u^0\) of \cref{fig:uhat}).
...
To show there is a useful (slow) centre manifold, we start with equilibria in the system (corresponding to \cref{thm:equil} on p.\pageref{thm:equil}).
...
\label{eq:equil1}
\end{equation}
The spectrum about each of these equilibria determine the manifold structure (corresponding to \cref{thm:dicho,lem:hs} on  pp.~\pageref{thm:dicho},\pageref{lem:hs}).
...
\cref{fig:simpevec} plots (solid) the two eigenfunctions corresponding to the two of these eigenvalues of smallest magnitudes.
...
\cref{fig:simpevec} plots (dashed) the two eigenfunctions corresponding to the two of these eigenvalues of smallest magnitude.
\end{itemize}
One of the beautiful properties of the coupling conditions~\eqref{eq:cc1} is that with them the diffusion operator~\(\DD x{}\) is self-adjoint (\cref{thm:sa}, p.\pageref{thm:sa}).
...
This useful self-adjointness is not a property of previous holistic discretisations \cite[Part~V, e.g.]{Roberts2014a}, with the exception of the unusual coupling proposed by \cite{Roberts08l}. 
Preserving self-adjointness is a new feature established by the new approach developed herein.
...
assures us that there exists a slow manifold in some neighbourhood of the subspace~\EE\ of equilibria (corresponding to \cref{thm:gsm}); that is, global in amplitude~\(U\) and local in parameters~\(\gamma\) and~\(\alpha\).
...
A comparison of approximations to the long-term, quasi-stationary, decay of the heat \pde: blue-solid, \(u\propto 1-|x|\) is the basic linear approximation~\eqref{eq:equil1}; red-dotted, the derived cubic spline~\eqref{eq:egsm1} at full coupling \(\gamma=1\); and, almost indistinguishable, brown-solid, is the exact mode \(u\propto\cos(\pi x/2)\).
}}
\end{tabular}
\end{figure}
...
\end{equation*}
The field~\(u\), plotted in \cref{fig:simp}, is an excellent cubic spline approximation to the correct~\(U\cos(\pi x/2)\) eigenfunction, also plotted in \cref{fig:simp}.
...
For example, modified computer algebra~\cite[Ch.~14]{Roberts96a, Roberts2014a} constructs the slow manifold plotted in \cref{fig:nonlinsm} on which the nonlinear evolution is
\begin{equation}
\dot U=-(3\gamma-\tfrac35\gamma^2)U-\tfrac1{15}\gamma^2\alpha^2U^3
+\Ord{\gamma^3+\alpha^3}.
\label{eq:nlsme}
\end{equation}
The nonlinear advection of Burgers' \pde\ generates steeper gradients in the subgrid field (\cref{fig:nonlinsm}) that enhance the decay.
...
We use centre manifold theory \cite[e.g.]{Carr81, Haragus2011} to newly establish (\cref{thm:gsm}) the in-principle existence and emergence of an exact closure to the dynamics of  \pde{}s in the class~\eqref{eq:genpde}.
...
\cref{thm:sa} establishes all eigenvalues of~\(\cL\) are real. 
...
\,,\label{eq:lem3a}
\end{align}
for bound~\(\beta\) defined by \cref{thm:dicho}.
...
By the next \cref{lem:hs}, \(-\lambda_1\geq\pi^2\), and so all non-zero eigenvalues satisfy \(-\lambda\geq\beta\) as required.
...
As illustrated by \cref{fig:leaspec}, for the specific case of the linearised problem~\eqref{eqs:lin} for which \(C(0)=1\)\,, there is a useful spectral gap because the smallest allowable nonzero nondimensional wavenumber is \(\kappa=\pi\)\,.
...
The proof of \cref{lem:hs} also applies to this case.
Inequality~\eqref{eq:kappa} then gives allowed wavenumbers for general~\(\gamma\).
As coupling parameter~\(\gamma\) varies from zero to one, the factor~\(C(\gamma)\) varies from one to zero, and so the denominator~\(C\) in inequality~\eqref{eq:kappa} increases the slope of the straight line of \cref{fig:leaspec}.
...
\begin{hypothesis}
The following \cref{thm:gsm} relies upon theorems by \cite{Haragus2011} which in turn require certain preconditions listed here that need to hold.
We consider twice differentiable, \(L\)-periodic, square integrable functions on~\(\tilde\XX\) which forms the requisite Hilbert spaces (\(\cZ\hookrightarrow\cY\hookrightarrow\cX=\LL\)).
Then the self-adjoint, linearised operator~\eqref{eqs:lin} of diffusion on a finite spatial domain forms an analytic semigroup \cite[Remark~2.18, e.g.]{Haragus2011}.
\begin{enumerate}[ref=Hypothesis~\thetheorem.\theenumi]
\item\label{hsd2p4} Spectral Decomposition Hypothesis~2.4 of \cite{Haragus2011}: consider the spectrum \(\sigma\) of the linear operator and assume that
\begin{enumerate}
\item there exists a constant~\(\beta>0\) such that
\begin{equation*}
\inf_{\{\lambda\in\sigma:\Re\lambda>0\}}\Re\lambda>\beta,
\quad\text{and}\quad
\sup_{\{\lambda\in\sigma:\Re\lambda<0\}}\Re\lambda<-\beta;
\end{equation*}
\item the set \(\{\lambda\in\sigma:\Re\lambda=0\}\) consists of a finite number of eigenvalues with finite algebraic multiplicity.
\end{enumerate}

\item\label{h2p1} Hypothesis~2.1 of \cite{Haragus2011}: (i)~\(\cL\in \mathscr L(\cZ,\cX)\); and 
(ii)~writing a system as \(du/dt=\cL u+R(u)\), for some \(q\geq2\) there exists a neighbourhood \(\cV\subset\cZ\) of the origin such that \(R\in C^q(\cV,\cY)\) and that \(R(0)=0\) and \(DR(0)=0\).

\item\label{h3p8ii} In the case of non-autonomous nonlinearity (\(R\)~becomes~\(R(u,t)\)),  Hypothesis~3.8(ii) of \cite{Haragus2011} additionally assumes that for any sufficiently small~\(\varepsilon\), there exist positive constants~\(\delta_0(\varepsilon)=\Ord{\varepsilon^2}\) and~\(\delta_1(\varepsilon)=\Ord{\varepsilon}\) such that
%\footnote{These equalities ``show that the nonlinear term~\(R\) is bounded with respect to all \(t\in\RR\), uniformly for~\(u\) in any sufficiently small closed ball~\(B_\varepsilon(\cZ)\).''}
\begin{equation*}
\sup_{u\in B_\varepsilon(\cZ)}\|R(u,t)\|_\cY=\delta_0(\varepsilon),\quad
\sup_{u\in B_\varepsilon(\cZ)}\|D_uR(u,t)\|_{\mathscr L(\cZ,\cY)}=\delta_1(\varepsilon).
\end{equation*}
\end{enumerate}
\end{hypothesis}

\begin{theorem}[slow manifold] \label{thm:gsm}
Consider the nonlinear \pde~\eqref{eq:genpde} on domain~\(\tilde\XX\) with coupling conditions~\eqref{eqs:cc} and assume that the functions~\(F\) and~\(G\) of the \pde~\eqref{eq:genpde} are smooth enough so that for every \(u^*\in\EE\) the nonlinear function \(R(u,t):=F(u^*_x+u_x)_x-\cL u+\alpha G(t,u^*+u)\) satisfies \cref{h2p1,h3p8ii}.
\begin{enumerate}
...
This slow manifold is emergent in the sense that for all solutions~\(u(x,t)\) of~\eqref{eq:genpde} and~\eqref{eqs:cc}, that stay in~\cE, there exists a solution~\(\Uv(t)\) of~\eqref{eq:gensm} such that \(u(x,t)=u(x,t,\Uv(t),\gamma,\alpha)+\Ord{e^{-\beta' t}}\) for decay rate \(0<\beta'<\beta\) (from \cref{thm:dicho}).
...
\cref{thm:dicho} establishes the Spectral Decomposition \cref{hsd2p4} \cite[from][]{Haragus2011} for all \(u^*\in\EE\).
%The functions~\(F\) and~\(G\) of the \pde~\eqref{eq:genpde} are assumed smooth enough so that the nonlinear function \(R(u):=F(u^*_x+u_x)_x-\cL u+\alpha G(u^*+u)\) satisfies \cref{h2p1}.
...
Because the centre eigenvalues are all zero (\cref{thm:dicho}), they are more precisely called \emph{local slow manifolds}.
...
\item The unstable spectrum is empty (\cref{thm:dicho}), so Theorem~3.22 of \cite{Haragus2011} applies to establish the exponentially quick emergence of the slow manifold to all solutions that remain within~\cE\ for all time.
By continuity in perturbations, the rate of attraction to the slow manifold in~\cE\ is estimated by the linearised rate~\(\beta\) at~\EE\  \cite[\S11.3, e.g.]{Roberts2014a}.
...
Then, for \(\alpha=\gamma=0\)\,, the subspace~\EE\ of piecewise linear equilibria of \cref{thm:equil} still exists.
Upon linearisation about each of these equilibria, the spatial differential operator \(\cL=\nu\DD x{}\) on~\(\tilde\XX\) remains self-adjoint (\cref{thm:sa}).
The exponential dichotomy of the operator~\cL\ (\cref{thm:dicho,lem:hs}) still applies, namely that there are \(N\)~eigenvalues of zero, and the others are\({}\leq-\beta\).
So far, the considerations are the same for both diffusion systems and wave systems.

The differences in theoretical support start with \cref{thm:gsm}.
...
Nonetheless, based upon recursively constructing coordinate transforms to a normal form \cite[Chap.~13]{Cox93a, Cox93b, Roberts2014a} we make the following `backwards' conjecture \cite[e.g.]{Grcar2011, Roberts2018a} for the wave \pde~\eqref{eq:wavpde}.
Parts of this conjecture for waves correspond to \cref{thm:gsm} for dissipative systems.
...
\emph{(A difference with \cref{thm:gsm}.3 is that here we posit a `nearby' approximating system and then base results on that.)}
\begin{enumerate}
...
\label{eq:wavsm}
\end{equation}
\emph{(A difference with \cref{thm:gsm}.1 is that here there are nearby systems which have slow manifolds; although like the dissipative case, such a nearby system does possess an exact low-dimensional closure.)}
...
\end{equation*}
\emph{(A difference with \cref{thm:gsm}.\ref{thm:gsm:emergent} is that here the slow manifold is not exponentially attractive and instead generally acts as a dynamical centre for nearby dynamics.)}
...
Burgers' \pde~\eqref{eq:burgers} is in the class~\eqref{eq:genpde} addressed by the theory of \cref{sec:lin} and so \cref{thm:gsm} assures us a slow manifold model exists.
...
Computer algebra code listed in the Ancillary Material (\cref{sec:aa}) confirms the following algebraic summary.
...
\end{itemize}
Combining these with the initial approximation gives the slow manifold and evolution thereon as
\begin{subequations}%
\begin{align}&
...
\end{align}
\end{subequations}
This slow manifold discretisation~\eqref{eq:dudt1} is discussed as~\eqref{eq:holistic1} in the Introduction.
...
%We demonstrate in \secref{numeric} how this stability arises in low-dimensional models.
...
Higher order terms in the asymptotic series for the slow manifold field~${u}$ may be systematically computed by iteratively solving equation~\eqref{eq:order-n} after having first computed~$\gv^n$ (as implemented in the computer algebra code of the Ancillary Material, \cref{sec:aa}). 
...
\label{eq:diffusion}
\end{align}
\cref{fig:diff:spectrum} shows that each additional term in this expansion provides a better approximation to the full continuum dynamics. In particular, since 
...
\begin{equation}
V_1=\frac{\mu(4-\mu\theta)}{8+2\mu(1-\theta)}\,, \quad
V_2=\mu\,, \quad
V_3=\frac{\mu(4+\mu\theta)}{8-2\mu(1-\theta)}\,,
\label{eq:mu-crit}
\end{equation}
where $\mu$ satisfies
\begin{equation}
...
Turning now to numerical simulation, we use a $2\pi$-periodic domain with $\nu=1$ and $\alpha=1$ for convenience. 
The initial field $u(x,0)=A\sin x$ is integrated at the $N+1$ grid-points $X_j=Hj$ for all $j\in\JJ$, with spacing $H=\frac{2\pi}{N}$. 
All time integration for this article was computed with \textsc{Matlab}'s \verb|ode15s|
The integration is performed for a maximum duration of $T=10$, but ceases early at the first sign of either: an instability, detected when $|U_j|>1000$ (denoted by~`\(\times\)'); or a non-monotonic  irregularity (denoted by~`+'). 
For each number~$N$ of discretised intervals, a search is made over values of~$A$, both positive and negative,
for which instability or irregularity first occurs, as plotted in \cref{fig:numeric1}.
\begin{figure}
\centering
\includegraphics{burgers_stab_plot4_100.pdf}
...
and thus proportional to the grid-spacing~$H$. None of the conservative, mixture or holistic models inherently outperforms the others in this measure.
...
diffusive \pde{}s described in \cref{sec:intro}.
In particular, it empowers centre manifold theory, discussed in \cref{sec:lin}, to use the \pde\ to inform iterative refinement from an initial approximation to a field~$u(x,t)$ whilst incorporating the macroscale dynamics of the relevant \pde{}. 
...
The use of a piecewise-linear initial approximation~$u^0$ to~$u$, discussed in \cref{sec:lin}, 
...
As discussed in the example application of \cref{sec:nonlin}, also interesting is that
...
\cref{sec:nonlin} illustrates that the induced discretisation follows directly from the \pde{}, as a function of the initial approximation~$u^0$. 
Furthermore, at least in the case of Burgers' \pde\ with a piecewise-linear initial approximation, \cref{sec:numeric} indicated that the holistic discretisation automatically favours numerically stable approximations. 
Indeed, the iterative refinement provided by the centre manifold framework acts to improve the order of approximation of spatial derivatives in terms of the discrete grid-spacing~$H$, as discussed in \cref{sec:nonlin}. 
...
AJR thanks the Australian Research Council for partial support of this project through grant DP150102385.
