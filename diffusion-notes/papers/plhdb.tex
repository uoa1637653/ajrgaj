...
The approach taken here is to start with a piecewise linear but continuous approximation,
...
%Letting $\xi=\sum_{j=1}^{N}\chi_j\xi_j$\ajr{Does not seem correct to me??} for convenience, 
...
Equation~\eqref{eq:order-n} becomes
\begin{eqnarray*}
...
\end{eqnarray*}
for backward difference $\nabla=1-\sigma^{-1}$.\gaj{Why not use $\Delta$?}
...
+ \Ord{\gamma^4}.
\end{eqnarray}
Figure~\ref{fig:diff:spectrum} shows that each additional term in this expansion provides a better approximation to the full continuum dynamics.
\begin{figure}
\centering
...
with spacing $H=\frac{2\pi}{N}$. The integration is performed for a maximum duration of $T=10$, but ceases early at the first sign of an instability, being either: a magnitude $|U_j|$ exceeding $1000$ (denoted by 'x'); or an non-monotonic 
irregularity (denoted by '+'). The smallest value of $A$ for which an instability occurs is plotted against a selection of 
values for the number $N$ of discretised intervals, as shown in Figures~\ref{fig:numeric1}--\ref{fig:numeric2}.
\begin{figure}
\centering
\includegraphics{figures/integ-stability-30.eps}
...
$\theta=\frac{2}{3}$ (mixture). Instability is detected either when $|U_j|>1000$ (denoted by 'x'), or by the occurrence of
non-monotonic modes (denoted by '+'). The plots show the smallest values of $A$ for which instability occurs
in the various models for each value of $N$.}
\label{fig:numeric1}
\end{figure}
\begin{figure}
\centering
\includegraphics{figures/integ-stability-60.eps}
\caption{Hmm?}
\label{fig:numeric2}
\end{figure}

Observe that for $N=3$ intervals, only the advective model ($\theta=0$) displays 
an instability, corresponding to the unstable critical point deduced above for $M=2$.
\gaj{Conservative model should also be unstable.}
Additionally, numerical instability does not occur for the advective model for $N=4$, corresponding to the lack of unstable critical points deduced above for $M=3$.\gaj{According to theory, the conservative model should be unstable for $M=2$.}
Overall, the onset of instability for the advective model occurs at lower amplitudes than for the other models,
which agrees with empirical observations \cite[e.g.]{Fornberg73}.
