...
With the aid of the holistic equations~\eqref{eq:temporal} and \eqref{eq:spatial}, 
...
by considering perturbations about the equilibrium $(\hat{u},\gamma,\alpha)=(\hat{u}_{0},0,0)$, where
$\hat{u}_{0}=\sum_{j=1}^{N}\chi_j{\cal I}_0 U_j$ with
${\cal I}_0=\xi+(1-\xi)\sigma^{-1}$.
In particular, we consider the power series
\begin{eqnarray}
...
and consider finite truncations of\eqref{eq:power}.
...
\end{eqnarray}
then $\hat{u}=\hat{u}^{<n>}+O(\gamma^{n+1}+\alpha^{n+1})$ and
