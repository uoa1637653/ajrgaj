...
\let\harvardurl\url
\usepackage{graphicx,amsmath,amsfonts,defns,moreverb}
\allowdisplaybreaks
\usepackage{pgfplots}
...
Typically, the modeller chooses arbitrary finite-element forms to approximate the various spatial derivatives.
In contrast, the holistic discretisation approach described herein obviates this choice, and leads
unambiguously to natural derivative approximations informed by the underlying microscale dynamics. 
The form of these approximations is determined in conjunction with the imposed
internal boundary conditions, which themselves are largely governed by the placement of the grid-points and the 
form of the leading discrete approximation.
The holistic process is commenced with a piecewise linear but continuous approximation,
in contrast to similar analyses that use piecewise constant, discontinuous approximations.
We are motivated by the principle that a more accurate leading approximation should lead to faster
convergence of the asymptotic solution, via the Rayleigh-Ritz theorem.
The stability of the resulting holistic discretisation of Burgers' \pde{} is compared to
that of other typical finite-element approximations.
\end{abstract}
...
\label{eq:genpde}
\end{equation}
where subscripts \(x\) and~\(t\) denote spatial and temporal derivatives, respectively, and for suitably smooth functions \(F\) and~\(G\).
Given \(N+1\)~discrete points in space, \(x=X_j\) for \(j=0,\ldots,N\)\,, we define grid values \(U_j(t)=u(X_j,t)\)\,.
...
Let the dynamics of the field~$u(x,t)$ be summarised by the coarse variables ${\Uv}=(U_1,U_2,\ldots,U_N)$, where these are grid values \begin{equation}
U_j(t):=u(X_j,t) \quad\text{for all }t\in \TT.
\label{eq:gridval}
\end{equation}
...
\begin{eqnarray}
	\dot U_j = g_j({\Uv}) \quad\text{for all }j
\label{eq:temporal}
\end{eqnarray}
...
For illustrative purposes, section~\ref{??} compares results with those of a convex combination of these two representations; that is, section~\ref{??}  compares results with Burgers' \pde~\eqref{eq:burgers} discretised to the so-called mixture model
\begin{equation}
	\dot{U}_j = 
...
The final part of the process is to express the physical field~\(u(x,t)\) in terms of the coarse variables~${\Uv}(t)$. 
...
The Rayleigh--Ritz theorem motivates our choice for a leading approximation that is a piecewise linear function (e.g., the blue~\(u^1\) of Figure~\ref{fig:uhat}), in contrast to the usual piecewise constant one \cite[]{Roberts98a, Roberts00a, Roberts2011a}.
...
Consequently, this article develops a systematic approximation to an in-principle exact discrete closure based upon systematically refining a piecewise linear and continuous subspace approximation to the field~\(u\).
...
In contrast, here we craft a coupling that moderates the communication between the two intervals, and then lets the \pde~\eqref{eq:burgers} itself tell us the appropriate fields and model.
...
\item Negative eigenvalues \(\lambda=-k^2\) arise corresponding to eigenfunctions of the form \(v\propto\sin[k(1-|x|)]\) which arises by necessity from the \pde, the homogeneous Dirichlet boundary conditions, and the continuity of~\(v\).
...
This useful self-adjointness is not a property of previous holistic discretisations \cite[Part~V, e.g.]{Roberts2014a}, but is a new feature maintained for the approach of this article.
...
\end{equation*}
Evidently the series in~\(\gamma\) appears to have a radius of convergence much larger than one\footnote{Construction of the slow manifold to 40th~order in~\(\gamma\) (for \(\alpha=0\)) followed by a generalised Domb--Sykes plot \cite[Appendix]{Mercer90} predicts convergence for all \(|\gamma|<3.8\)\,---the  convergence limiting singularity lying at an angle~\(103^\circ\) to the real \(\gamma\)-axis.}.
...
\end{equation*}
Considering together the two mapping equations, this spatial map has solutions \((A_j,B_j)\propto \mu^j\) for some multiplier~\(\mu\) only for vanishing determinant
\begin{eqnarray}&&
...
\item Given two smooth functions \(\tilde u(x,\Uv,\gamma,\alpha)\) and~\(\tilde\gv(\Uv,\gamma,\alpha)\) for the governing equations~\eqref{eq:genpde}+\eqref{eqs:cc}, evaluated at \(u=\tilde u\) such that \(d\Uv/dt=\tilde\gv\), having residuals~\Ord{\gamma^p+\alpha^q} as \((\gamma,\alpha)\to\vec 0\)\,,  then the slow manifold is 
\begin{equation*}
...
\section{Nonlinear modelling of Burger's PDE}\label{sec:nonlin}
...
%equation~\eqref{eq:man},  the terms ${u}^{(p,q)}$ and $\gv^{(p,q)}$ are computed subject to the coupling conditions~\eqref{eqs:cc}.
%To simplify the algebraic construction process we follow the approach of \cite{Jarrad2001}:
%without loss of generality, consider finite truncations of the power series subject to $p+q\le n$,
%namely\footnote{Other truncation patterns may be used, such as,
%for example, the rectangular truncation $p\le n$\,, $q\le m$\,.}
%\ajr{I think we can reduce the plethora of symbols by introducing varepsilon a tad earlier and using it to order the sums, and it alone.}
...
\end{eqnarray*}
\begin{itemize}
\item The interelement continuity condition~\eqref{eq:ccc} requires that ${u^n}(X_j,\Uv)=0$ for $n=1,2,3,\ldots$,
because ${u^0}(X_j,\Uv)=U_j=u(X_j,\Uv)$. 
Hence, we solve for~$d_j$ at $\xi_j=0$ and
$c_j$ at $\xi_j=1$, giving
\begin{eqnarray}
...
\item The interelement smoothness condition~\eqref{eq:ccj} determines the value of~$g^1_j$.
...
& = & 0 \hspace*{2mm}\mbox{ for } n=2,3,\ldots\,.
\end{eqnarray}

The higher order advection terms in $\alpha$ and the interactions between $\alpha$ and $\gamma$
rapidly become more complex. For example, the $\gamma\alpha$ terms are 
\begin{eqnarray}
...
convenience. The initial field $u(x,0)=A\sin x$ is integrated at the $N+1$ grid-points $X_j=Hj$, $j=0,1,\ldots,N$,
...
$\theta=\frac{2}{3}$ (mixture). Instability (detected when $|u|>1000$) is denoted by `x', and irregularity
...
that for $N=4$ none of the simulated models show instability, nor irregularity.
...
and thus proprtional to the grid-spacing $H$. None of the conservative, mixture or holistic models
inherently outperforms the others.
...
incorporating the dynamics the relevant \pde{}. The resulting approximation is a function of the discrete grid values
$U_j(t)$, the \pde{} parameters, and an introduced homotopy parameter $\gamma$ that controls
...
In conjunction with the inner continuity and smoothness conditions, for instance,
the self-adjointness of the diffusion operator $\cL$ is preserved under periodic, Dirichlet or Neumann outer boundary conditions. This is true for all $\gamma\in[0,1]$, although the usual Neumann condition holds exactly only for $\gamma=1$
and requires modification for $\gamma<1$ (governed by the chosen smoothness condition).
Also of interest is the fact, noted in Section~\ref{sec:nonlin}, that
...
the coarse dynamics. Holistic analysis of Burgers' equation shows that the derived approximation to $u$ only achieved
$C^2$ continuity even for high orders of $\gamma$. There remains scope for research into finding
an alternative form of the
smoothness condition that would lead to $C^n$ smoothness for $\Ord{\gamma^{n+1}}$ approximations.

Another useful facet of holistic discretisation is that it eliminates the ambiguity inherent in choosing appropriate
discrete approximations to the spatial derivatives in the \pde{}. As demonstrated in Section~\ref{sec:nonlin}, the induced discretisation follows directly from the \pde{}, as a function of the initial approximation $u^0$. 
Furthermore, at least in the case of Burgers' equation with a piecewise--linear initial approximation,
the holistic discretisation appears to automatically favour numerically stable approximations, as shown
in Section~\ref{sec:numeric}. 
Indeed, the iterative refinement provided by the holistic procedure acts to improve the order of approximation of spatial derivatives in terms of the discrete grid-spacing $H$,
as discussed in Section~\ref{sec:nonlin}. This is somewhat akin to the process of deriving a geometric integration scheme, but applied spatially rather than temporally.
...
Finally, it should be noted that the models examined in this paper were restricted to a general class of one-dimensional reactive-advection-diffusion \pde{}s. 
A proposed area of research 
is to extend piecewise-linear holistic discretisation to nonlinear wave models, since the self-adjointness of $\cL$
would still apply to ensure real-valued wave numbers. Another active research direction is the extension of
piecewise-linear holistic discretisation to two or more spatial dimensions. In general, there is exciting scope for exploring
