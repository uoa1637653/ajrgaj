...
\end{eqnarray*}
with centred difference $\delta=\sigma^{1/2}-\sigma^{-1/2}$,
...
\label{eq:holistic1}
\end{eqnarray}
with nonlocal operator $S=(1+\delta^2/6)^{-1}$. 
Observe that, apart from $S$,
...
For a self-adjoint operator~\cL, the Rayleigh--Ritz theorem is that an eigenvector~\vv\ with error~\Ord{\epsilon}
has a corresponding eigenvalue $\lambda = {\langle{\vv},{\cL}{\vv}\rangle}/{\|{\vv}\|^2}$ with asymptotically smaller error~\Ord{\epsilon^2}.
...
We seek linearised solutions \(u\approx (1-|x|)U+e^{\lambda t}v(x)\) for small~\(v\):  the diffusion \pde~\eqref{eq:burgers} becomes the eigenproblem
\begin{equation}
-v_{xx}+\lambda v=0\,,
...
where $\hat{u}_{0}=\sum_{j=1}^{N}\chi_j((1-\xi_j)U_{j-1}+\xi_jU_j)$; recall that $\chi_j$
is a $0$-$1$ indicator for $x\in\XX_j$.
...
\begin{eqnarray}
 {\cR}(\hat{u}^{\langle n\rangle},\gv^{\langle n\rangle}) = \Ord{\varepsilon^{n+1}}
%\,,
%\end{eqnarray}
%or, equivalently,
%\begin{eqnarray}
...
Letting $\xi=\sum_{j=1}^{N}\chi_j\xi_j$ for convenience, the first-order pertubation terms 
