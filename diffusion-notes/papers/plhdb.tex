...
unstable, nontrivial critical points occur when $0<\theta<\theta_c$.
...
Interestingly, the purely advective form ($\theta=0$) is now also unconditionally stable for $M=3$.
...
continuity and smoothness. As found in Section~\ref{sec:nonlin}, this iterative refinement acts to improve
the order of approximation of spatial derivatives in terms of the discrete grid-spacing $H$.

Additionally, the use of a piecewise--linear initial approximation to $u$, discussed in Section~\ref{sec:lin}, permits the diffusion operator $\cL$ to be
self-adjoint under conventional boundary conditions (periodic, Dirichlet and Neumann). This in turn limits the possible eigenmodes to having real eigenvalues. \gaj{Not sure what I'm trying to say here.} Furthermore, 
the piecewise--linear approximation plus its 
first-order correction from the diffusion equation is an exact cubic spline representation of $u$ in terms of $U_j$
(as noted in Section~\ref{sec:nonlin}). Notably, the cubic spline is defined in terms of the nonlocal operator $S$, which appears naturally in the holistic derivation of the coarse dynamics.

Lastly, holistic discretisation eliminates the ambiguity inherent in choosing appropriate
discrete approximations to the spatial derivatives in the \pde{}. As demonstrated in Section~\ref{sec:nonlin}, the induced discretisation follows directly from the \pde{}, as a function of the initial approximation. 
