...
back to the continuum field $u(x,t)$ via some approximate spatial mapping of the form
\begin{eqnarray}
	u(x,t)  \sim \hat{u}(x,{\vec U}(t)).
	\label{eq:spatial}
\end{eqnarray}
We show in Section~\secref{nonlin} how this mapping is holistically constructed  by use of centre manifold theory.
**TODO show equation.
...
\begin{eqnarray}
     \hat{u}_0 = \sum_{j=1}^{N}\chi_j(\xi_j U_j+(1-\xi_j)U_{j-1})
% = \sum_{j=1}^{N}\chi_j(\xi_j+(1-\xi_j)\sigma^{-1})U_j
\,,
\label{eq:uhat0}
\end{eqnarray}
with indicator $\chi_j(x)=1$ (else 0) for $X_{j-1}\le x\le X_j$, and linear interpolator
$\xi_j(x)=(x-X_{j-1})/H$. This enforced continuity is represented by internal boundary conditions
(IBCs) of the form 
\begin{eqnarray}
   [u]_j = 0 && \mbox{for } j=1,2,\ldots,N-1\,,
\label{eq:cont-cond}
\end{eqnarray}
...
$[\D x{\hat{u}_0}]_j=\delta^2U_j/H$. Hence, we introduce a homotopic smoothing parameter $\gamma$
($0\le\gamma\le 1$),
and enforce the additional IBCs: %smoothness condition that
\begin{eqnarray}
   [u_x]_j = \frac{1-\gamma}{H}\left.u\right|_{X_j} && \mbox{for } j=1,2,\ldots,N-1\,,
\label{eq:smooth-cond}
\end{eqnarray}
such that smooth approximations are found in the limit as $\gamma\rightarrow 1$.
Consequently, the state-space is now extended to $(u,\alpha,\gamma)$, for which $(\hat{u}_0,0,0)$ is
...
We seek the spatial stability over $\XX$, allowing for discretisation, of the eigenmode
\begin{eqnarray}
v = \sum_{j=1}^{N}\chi_j \Re\left(a_j e^{i\kappa\xi_j}\right)\,,
\end{eqnarray}
...
\end{axis}
\end{tikzpicture}
\caption{The equilibrium ($\gamma=0$) spectrum determined by the forbidding condition~\eqref{eq:leaforbid}.
The thick lines along the $\kappa$ axis indicate regions of stability.}
\label{fig:leaspec}
\end{figure}
