...
if the general cubic spline in the $j$th interval is specified as 
\(S_j(x)=a_jH^3\xi_j^3/6+b_jH^2\xi_j^2/2+c_jH\xi_j+d_j\), 
then its second derivative at the left-hand end of the interval
is given by $b_j=S\delta^2 d_j/H^2$ \cite[e.g.]{BurdenFaires85}.
As $d_j=U_{j-1}$ in our example, this gives \(\left.\DD x{U_{j}}\right|_{X_j}=S\delta^2 U_j/H^2\), and
so the holistic approach ensures the first-order correction, derived above, forms a cubic spline. 
