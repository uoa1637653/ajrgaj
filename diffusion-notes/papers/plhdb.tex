...
\begin{equation}
\cR(u,\gv)=0 \quad\text{where the residual }
...
\end{cases}
\end{equation*}
We seek the slow manifold for the coupled and nonlinear dynamics in the multivariate power series form
\begin{eqnarray}
  {u} \sim \sum_{p=0}^{\infty}\sum_{q=0}^{\infty}\gamma^{p}\alpha^{q}{u}^{(p,q)}\,,
&&
...
\begin{eqnarray}
   {u}^{\langle n\rangle} = \sum_{p=0}^{n}\sum_{q=0}^{n-p}\gamma^{p}\alpha^{q}{u}^{(p,q)}\,,
&&
   {u^n} = \sum_{p=0}^{n}\gamma^{p}\alpha^{n-p}{u}^{(p,n-p)}\,.
\end{eqnarray}
Defining $\varepsilon=\sqrt{\gamma^2+\alpha^2}$ for convenience,\footnote{
This is merely an algebraic device; in practice, the small parameters~$\gamma$ and~$\alpha$ need not be commensurate.} then we can write
 ${u}={u}^{\langle n\rangle}+\Ord{\varepsilon^{n+1}}$ and
${u}^{\langle n\rangle}={u}^{\langle n-1\rangle}+u^n$; 
likewise for $\gv^{\langle n\rangle}$ and $\gv^n$.
Substituting these into~\eqref{eq:man} and rearranging we can deduce
\begin{eqnarray}
 {\cR}({u}^{\langle n\rangle},\gv^{\langle n\rangle}) = 
  -\D {\Uv}{{u^0}}\cdot\gv^n +\nu\DD x{u^n}+{\cR}({u}^{\langle n-1\rangle},\gv^{\langle n-1\rangle})
+\Ord{\varepsilon^{n+1}}\,.
\end{eqnarray}
Hence to require \({\cR}({u}^{\langle n\rangle},\gv^{\langle n\rangle}) = \Ord{\varepsilon^{n+1}}\), the process is to iteratively solve
\begin{equation}
\nu\DD x{u^n} = \D {\Uv}{{u^0}}\cdot\gv^n-{\cR}({u}^{\langle n-1\rangle},\gv^{\langle n-1\rangle})
%+\Ord{\varepsilon^{n+1}}
\,.
\label{eq:order-n}
\end{equation}

Letting $\xi=\sum_{j=1}^{N}\chi_j\xi_j$\ajr{Does not seem correct to me??} for convenience, the first-order perturbation terms 
can now be computed for $n=1$ as
\begin{eqnarray*}
\nu\DD x{u^1} = \sum_{j=1}^{N}\chi_j\left[(\xi+(1-\xi)\sigma^{-1})g_{1,j}
   +\alpha(\xi+(1-\xi)\sigma^{-1})U_j\frac{1}{H}\nabla U_j\right]\,,
\end{eqnarray*}
for backward difference $\nabla=1-\sigma^{-1}$.
Spatially integrating twice gives
\begin{eqnarray*}
\nu{u^1} & = &  \sum_{j=1}^{N}\chi_j\left[d_j+H\xi c_j+\frac{H^2}{6}(\xi^3+(1-\xi)^3\sigma^{-1})g_{1,j}
\right.\nonumber\\
&& \left.{}+\frac{\alpha H}{6}(\xi^3+(1-\xi)^3\sigma^{-1})U_j\,\nabla U_j\right]\,.
\end{eqnarray*}
Now, the continuity condition~\eqref{eq:ccc} ensures that ${u^n}(X_j,t)=0$ for $n>0$,
since ${u^0}(X_j,t)=U_j(t)=u(X_j,t)$. Hence, we solve for $d_j$ at $\xi=0$ and
$c_j$ at $\xi=1$, giving
\begin{eqnarray*}
\nu{u^1} & = &  \sum_{j=1}^{N}\chi_j\left[\frac{H^2}{6}(\xi^3+(1-\xi)^3\sigma^{-1}
-\xi\nabla-\sigma^{-1})g_{1,j}
\right.\nonumber\\
...
whence
\begin{eqnarray}
 {u} = \sum_{j=1}^{N}\chi_j\left[{\cI}_0 U_j+\frac{H^2}{6\nu}{\cI}_1\dot{U}_j
+\frac{\alpha H}{6\nu}{\cI}_1 U_j\,\nabla U_j\right]+\Ord{\gamma^2+\alpha^2}\,,
\end{eqnarray}
since $\dot{U}_j=g_{1,j}+\Ord{\gamma^2+\alpha^2}$. 

Next, the value of $g_{1,j}$ can be found
from the smoothness condition~\eqref{eq:ccj}, namely
\begin{eqnarray*}
\nu[{u^1_x}]_{j} = -H\left(1+\frac{1}{6}\delta^2\right)g_{1,j} 
...
\end{eqnarray}
where $S=(1+\delta^2/6)^{-1}$.
Observe that, apart from the nonlocal operator $S$, this is just the mixture model~\eqref{eq:mixture}
...
Higher order terms in ${u^n}$  may be systematically computed by iteratively solving equation~\eqref{eq:order-n} after
having first computed $\gv^n$. The latter can be found by applying the weak solvability condition \cite[]{Jarrad2001},
...
\begin{eqnarray}
\left.\frac{\nu\gamma}{H}\delta^2u^{n-1}\right|_{X_j} = HS^{-1}g_{n,j}
-\langle {\cR}({u}^{\langle n-1\rangle},\gv^{\langle n-1\rangle}), \hat{v}_0 \rangle\,.
\end{eqnarray}
Note that for $n=1$, this  reduces to equation~\eqref{eq:g1}, and
for $n>1$, the left-hand side is identically zero.
...
+ \Ord{\gamma^4}.
\end{eqnarray}
Figure~\ref{fig:diff:spectrum} shows that each additional term in this expansion provides a better approximation to the continuum dynamics.
\begin{figure}
\centering
...
\section{Stability of the discretisation}\label{sec:numeric}
...
except for $M$ adjacent, internal points. For example, for $M=2$ it suffices to choose $N=3$ (for a periodic domain) with
...
Interestingly, the purely advective form ($\theta=0$) is now also unconditionally stable.
