...
\begin{eqnarray}
	...
	\label{eq:diff-adv}
\end{eqnarray}
Under the assumption that $u$ still obeys the continuity condition~\eqref{eq:cont-cond} and the smoothness condition~\eqref{eq:smooth-cond}, this equation may be transformed, via the inner product of terms with the
linear dual solution $\hat{v}_0^{[j]}$, from equation~\eqref{eq:vhat0}, into
\begin{eqnarray}
	\left\langle\D {\vec U}u,\hat{v}_0^{[j]}\right\rangle\cdot\dot{\vec U} & = & 
...
Substituting the linear approximation $u=u_0+{\cal O}(\gamma+C)$, from equation~\eqref{eq:uhat0}, then leads to
\begin{eqnarray}
	H\left(1+\frac{1}{6}\delta^2\right)\dot{U}_j & = & 
\frac{\gamma}{H}\delta^2U_j - C\mu\delta U_j + {\cal O}((\gamma+C)^2)\,,
\end{eqnarray}
