...
Alternatively, recall from Section~\ref{sec:eig:modes} that for the special eigenmode $k=\frac{\pi}{H}$,
the corresponding continuum eigensolution $\tilde{u}$ decays at the rate $\lambda=-\frac{\pi^2}{H^2}$.
In contrast, the equivalent sawtooth mode in the coarse dynamics is given by $\tilde{U}_j=e^{\lambda t}(-1)^{j}$, 
assuming for the sake of argument that $X_j\equiv jH$. It can then be shown that $\tilde{U}_j$,
which obeys the relation $\delta^2\tilde{U}_j=-4\tilde{U}_j$,
satisfies equation~\eqref{eq:Udot:1} for $\gamma=1$ with a decay rate of $\lambda=-\frac{12}{H^2}$.
Observe that this is exactly the left-hand side of equation~\eqref{eq:spec:1st} for $c=-1$.
...
completely eliminated by the second-order correction, in favour of an $O(H^4k^6)$ error. In addition, as per Section~\ref{sec:first-order},
the sawtooth mode (with $c=-1$) now has a decay rate of $-\frac{48}{5H^2}=-\frac{9.6}{H^2}$, in comparison to the true value of
$-\frac{\pi^2}{H^2}\approx-\frac{9.87}{H^2}$ and the first-order value of $-\frac{12}{H^2}$.

The behaviour of this corrected, approximate spectrum is shown in Figure~\ref{fig:spec:2nd-order}.
Observe that the approximation becomes untenable near to the first turning point, at which $k\approx\frac{\pi}{H}$.
%% \cos kH \sim 1-k^2H^2/2+k^4H^4/24-O(k^6H^6).
%% -2(1-\cos kH) \sim -k^2H^2+k^4H^4/12-...
%% 1/(2+\cos kH) \sim 1/3(1-k^2H^2/6+...) = (1/3)*(1+k^2H^2/6-...)
%% -6(1-\cos kH)/(2+\cos kH) \sim (-k^2H^2+k^4H^4/12+...)*(1+k^2H^2/6+...)  = -k^2H^2-k^4H^4/6+k^2H^2/12+...
