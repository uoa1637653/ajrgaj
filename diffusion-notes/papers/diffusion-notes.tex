...
Next, the spatial approximation is improved to become\footnote{Now using an implicit dependence 
upon $\gamma$ and $C$, in contrast to the explicit dependence assumed in expansions~\eqref{eq:u:series} and~\eqref{eq:Udot:series}.} 
$\hat{u}=\hat{u}_0+\hat{u}_1+{\cal O}((\gamma+C))^2$, where $\hat{u}_1$ is found by solving
the approximate manifold equation
\begin{eqnarray}
{\cal M}(\hat{u},\hat{g}) & \approx & {\cal M}(\hat{u}_0+\hat{u}_1,\hat{g})~=~0+O((\gamma+C)^2)\,,
\end{eqnarray}
or, equivalently,
\begin{eqnarray}
{\cal L}\hat{u}_1 & = & -{\cal M}(\hat{u}_0,\hat{g})+O((\gamma+C)^2)\,.
\end{eqnarray}
Subsequently, the temporal approximation is improved by resolving the approximate solvability condition
\begin{eqnarray}
{\cal S}(\hat{u},\hat{g}) & \approx & {\cal S}(\hat{u}_0+\hat{u}_1,\hat{g})~=~0+{\cal O}((\gamma+C)^3)\,,
\end{eqnarray}
giving rise to
\begin{eqnarray}
	g_{j} & = & 
...
-\frac{2\gamma C}{5}(S-1)^2S\frac{\mu\delta U_j}{H}
\nonumber\\&&
{}-\frac{C^2}{20}(S-1)^2(2S-1)U_j+ {\cal O}((\gamma+C)^3)
\,.
\end{eqnarray}

\gaj{How to draw true spectrum $\lambda=-k^2-ikC$ or $(\lambda+k^2)^2+(kC)^2=0$?}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\appendix
\Alph{section}
