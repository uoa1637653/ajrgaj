...
in the form of a nonlinear advection-diffusion equation
\begin{eqnarray}
	\D tu = \DD xu - u\D xu\,,
\end{eqnarray}
which is just the viscous Burgers' equation with unit viscosity.
Observe that, unlike for the linear advective system~\eqref{eq:diff-adv}, 
the approximation $u\sim u_0$ from equation~\eqref{eq:uhat0} is not a stationary solution of 
the nonlinear system, since $u_0u'_0\ne 0$ in general.
This problem may be addressed by introducing a new expansion parameter $\varepsilon$ by which to modify the nonlinear term,
in much the same way that the parameter $\gamma$ was introduced in Section~\ref{sec:???}
to handle discontinuity. Consequently, the modified system is
\begin{eqnarray}
	\D tu = \DD xu - \varepsilon u\D xu\,,
	\label{eq:AD}
\end{eqnarray}
such that $(u,\gamma,\varepsilon)=(u_0,0,0)$ is an equilibrium solution.
...
\label{eq:Mug:AD}
\end{eqnarray}
%Given that $u$ obeys the continuity condition~\eqref{eq:cont-cond} and the smoothness condition~\eqref{eq:smooth- cond}, taking inner products with $\hat{v}_0^{[j]}$ leads
