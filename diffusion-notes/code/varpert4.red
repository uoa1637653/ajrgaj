...
procedure MM(u,g,h); df(u,x,x)+(1-epsilon)*u-(df(u,a)*g+df(u,b)*h)/epsilon-u*df(u,x);
...
begin
  ...
  write ghat:=ghat+gn;
  hhat:=hhat+hn;
  write aadot:=((a*ghat+b*hhat)/aa where {epsilon=>1,a^2+b^2=>aa^2});
end;
